function [ epsilon, sigv2, spec, Sthresh, Snoise ] = dissipation_spectr_acov( Dv, Urel, fs )
...
if nt >= dswin / 4 % skip to next iteration if window too small
    ...
    knoise = deal( zeros(nk, 1) );
    
    for k = 1 : nk % loop through vertical levels
        S(:) = fast_acov_spectr(w(:,k), nwin); % Ignore Nyquist-frequency power estimate!
        if isfinite(S(1))
            spec(:, kk(k)) = S;
            [Sthresh(kk(k)), Snoise(kk(k)), knoise(kk(k))] = HildebrandSekhon3(S, z1side);
