...
% [sthr,meannoise,kthr]=HildebrandSekhon(F,S)
...
% Basic idea: exclude large spectral estimates at low frequency until the remainder
% cannot be distinguished from white noise.
% My method includes lower and lower frequency spectral estimates until one can be
% distinguished from white noise to the right (higher frequency) of it.
...
% signal. Now ffind the end of white noise by assuming it
...
%p1side = normcdf(z1side);
%z2side = norminv( 1-(1-p1side)/2 );
...
% Better threshold:
% Find index of SE, to left of which (at lower wavenumber) 
...
kthr=firstnoise-1; % last _non_noise SE index, S(kthr)>noise but S(kthr+1)<noise.
...
meannoise=mean(s(1:ithr-4)); % SPdeS 2020-08-16: use ordinary mean rather than mean of median 5 points

% Another way to get the 95%ile of the noise
% This doesn't assume Gaussian distribution of noise,
% although white noise ought to be Gaussian.
% p1side = 0.95; 
% quantile(p1side, s(1:ithr-4))
% quantile(p1side, S((kthr+4):end)
