...
"""
function read_streamlinexr_stare!(file_path, h, beams, nheaderlines=17; skip_beams=0, nbeams=0, nbeams0=0)
    # nheaderlines can be incremented by the caller to skip beams
    # nbeam is the number of beam data requested read from the file
    # nbeam0 is offset index for the output dict
    ...
    # maximum number of beams the file contains
    totnbeams = round(Int, (nlines-nheaderlines) / (1+ngates)) # = nrays*ntimes
    nbeams = min(nbeams)
...
# add skip_lines=ship_beams*(1+ngates)
# end_beam=0 goes exactly to the end
function read_streamlinexr_stare(file_path::AbstractString, nheaderlines=17; 
    start_beam=1, end_beam=0)
    # end_beam=0 reads exactly to end,
    # negative end_beam=-n trims off n beams from the end,
    # positive end_beam reads to that index, e.g. beams[... :end_beam]
    ...
    # beams could be rays or times
    # totbeams = all beams in file
    totbeams = round(Int, (nlines-nheaderlines) / (1+ngates)) # = nrays*ntimes
    end_beam = mod(end_beam + totbeams - 1, totbeams) + 1
    # nbeams = number of beams to be read from this file
    nbeams = end_beam - start_beam + 1
    # start_beam default is 1
    # skip_beams = start_beam - 1 # do not compute skip_beams here
    #!!!!!
    # pass the absolute beam index start_beam 
    # to read_streamlinexr_stare!(), where it will be compared with 
    # the total cumulative beam index, to compute the skip_beams
    # Confusing because there is scalar and vector read_streamlinexr_stare() and
    # scalar and vector modifying read_streamlinexr_stare!().
    ...
    read_streamlinexr_stare!(file_path, h, beams, nheaderlines; skip_beams=skip_beams, nbeams=nbeams)
    return beams, h
end

"read multiple files"
function read_streamlinexr_stare(file_path::AbstractVector{T}, nheaderlines=17; 
    start_beam=1, end_beam=0 ) where {T<:AbstractString}
    # loop over a vector of files.
    
    # start_beam default is 1
    skip_beams = start_beam - 1

    # use header information in h, count lines in all files
    nfiles = length(file_path)
    nbeams = zeros(Int32, nfiles)
    ngates = -1 # initialize with a garbage value
    ...
    nbeams0 = 0 # cumulative total number of beams read
    for (i,file) in enumerate(file_path)
        h = read_streamlinexr_head(file) # reread header for each file
        read_streamlinexr_stare!(file, h, beams, nheaderlines; skip_beams=skip_beams, nbeams0=nbeams0) # updates beams[keys][nbeams0 .+ (1:nbeams[i])]
        nbeams0 += nbeams[i] # total number of beams now read
    end
...
function read_streamlinexr_beam_timeangles(file_path::AbstractVector{T}, nheaderlines=17) where {T<:AbstractString}
    ...
    for (i,file) in enumerate(file_path)
        ...
        beams[:nbeams][i] = nbeams[i]   # number of beams in this file
