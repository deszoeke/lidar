...
## Corpus Check
- 46 files · ~48,857 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 539 nodes · 781 edges · 52 communities (38 shown, 14 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 23 edges (avg confidence: 0.87)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `86523cb8`
...
- read_lidar
- LidarVNSync
- lidar_turbulence_cleanup.jl
- NoaaDas
- timing_lidar
- DopplerTurbulence
- Saved JLD Output Variables (QC doc)
- run_sequential_offsets
...
- Agent Init Table
- Graphify Workflow (Copilot Instructions)
- iterative_heave_rain.jl
- lidar_uv.jl
- chunk_lidar_datetimes
- init_stream_state
- 18s GPS-UTC Leapsecond Offset (May-June 2024)
- graphify
- save_stare_mdv_sync_pass2.jl
- Alex
- FnISO and Cyclone Remal (2024)
- Leah
- Caveman Mode (AGENTS.md)
- lidar micromamba/conda Environment (AGENTS.md)
- Persona Routing: Orchestrator to Planner to Editor to Tester
- Graphify Deps Check Hook
- Lidar Env Wrapper Hook
- extend_pycall_missing.jl
- trigs
- EKAMSAT ABL Working Group Meeting (2026-04-09)
- CLAUDE.md - Project Instructions
- process_sync_data

## God Nodes (most connected - your core abstractions)
1. `LidarVNSync` - 76 edges
2. `DopplerTurbulence` - 45 edges
3. `timing_lidar` - 29 edges
...
- `Sync Failure Modes & Prior Handling` --semantically_similar_to--> `Prior-Offset Double-Count Bug`  [INFERRED] [semantically similar]
  TODO.md → .github/plans/EXECUTIVE_SUMMARY_OFFSET_FIX.md
...
### Community 1 - "LidarVNSync"
Cohesion: 0.07
Nodes (22): fcn3(), chunks, Dates, DSP, FFTW, Interpolations, JLD2, NCDatasets (+14 more)

### Community 2 - "lidar_turbulence_cleanup.jl"
Cohesion: 0.06
...
### Community 4 - "timing_lidar"
Cohesion: 0.10
Nodes (21): ba(), cross_correlation(), dtregress(), DVM(), find_lags_iterative(), gps2dt(), gps2utc(), gpstime2gpsvndt() (+13 more)

### Community 5 - "DopplerTurbulence"
Cohesion: 0.07
Nodes (34): Module Consolidation into DopplerTurbulence.jl, DisplacementUncertainty, allcross(), anom(), D2_rho_small_disp(), D2_rho_stare(), D2_rho_stare_qc(), displacements() (+26 more)

### Community 6 - "Saved JLD Output Variables (QC doc)"
Cohesion: 0.11
...
### Community 7 - "run_sequential_offsets"
Cohesion: 0.29
Nodes (10): Sync-Quality Gates (AGENTS.md), Sync-Quality Gates (CLAUDE.md), Prior Reset Strategy (carry_forward / reset_each_file / reset_on_bad_quality), backward_jump_robustness(), is_jump_candidate(), prior_from_history(), robust_center(), run_sequential_offsets() (+2 more)
...
### Community 9 - "extract_sync_window"
Cohesion: 0.22
Nodes (14): diagnostic_example_chunks(), dt_seconds(), ensure_chunk_loaded!(), extract_sync_window(), fill_short_nan_gaps(), finite_overlap(), finite_overlap_corr(), one_hz_average_at_lidar_times() (+6 more)

### Community 10 - "vectornav2jld.jl"
Cohesion: 0.13
Nodes (9): ShipPosmv, Dates, Interpolations, JLD2, NoaaDas, Pkg, read_vecnav, Revise (+1 more)

### Community 11 - "time_tests.jl"
Cohesion: 0.17
...
### Community 19 - "coarse_and_fine_lag"
Cohesion: 0.31
...
### Community 32 - "iterative_heave_rain.jl"
Cohesion: 0.60
Nodes (4): infer_heave_from_stare(), Statistics, local_vertical_stats(), weighted_median()

### Community 33 - "lidar_uv.jl"
Cohesion: 0.40
Nodes (4): NCDatasets, Pkg, PythonPlot, Statistics

### Community 34 - "chunk_lidar_datetimes"
Cohesion: 0.67
Nodes (4): check_chunk_alignment_contract(), chunk_lidar_datetimes(), ensure_chunk_loaded_nc!(), load_chunk_for_dissipation()

### Community 35 - "init_stream_state"
Cohesion: 0.29
Nodes (8): diag_array(), diagnostic_single_window(), init_periodic_beams(), init_stream_state(), load_lidar_indices_and_files(), setup_sync_context(), setup_sync_context_nc(), write_mdv_sync_pass2!()

### Community 36 - "18s GPS-UTC Leapsecond Offset (May-June 2024)"
Cohesion: 0.50
...
### Community 51 - "process_sync_data"
Cohesion: 0.25
Nodes (8): LidarVNSync Core API Conventions (AGENTS.md), LidarVNSync Core API Conventions (CLAUDE.md), Prior-Offset Double-Count Bug, Offset Composition Fix (single-residual rule), append_nan_offset_log(), offset_or_sentinel(), offset_to_ms(), process_sync_data()

## Knowledge Gaps
- **145 isolated node(s):** `check-graphify-deps.sh script`, `run-in-lidar-env.sh script`, `bash`, `Revise`, `Pkg` (+140 more)
...
- **Why does `LidarVNSync` connect `LidarVNSync` to `chunk_lidar_datetimes`, `init_stream_state`, `run_sequential_offsets`, `extract_sync_window`, `process_sync_data`, `coarse_and_fine_lag`?**
  _High betweenness centrality (0.022) - this node is a cross-community bridge._
- **Why does `Module Consolidation into DopplerTurbulence.jl` connect `DopplerTurbulence` to `DisplacementUncertainty`, `Saved JLD Output Variables (QC doc)`?**
...
- **Should `lidar_turbulence_cleanup.jl` be split into smaller, more focused modules?**
  _Cohesion score 0.058823529411764705 - nodes in this community are weakly interconnected._
- **Should `timing_lidar` be split into smaller, more focused modules?**
